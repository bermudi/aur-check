...
**Source:** GitHub issue [#22](https://github.com/bermudi/aur-check/issues/22)
(Cohort 2, low L5)
**Status:** fixed
**Severity:** low
...
validation layer but was rejected by `aur-safe check`/`audit`/`explain`/gate,
...
- `./aur-safe selftest </dev/null` → 304 passed, 0 failed, including the new
