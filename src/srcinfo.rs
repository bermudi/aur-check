...
/// Real pacman-backed implementation. Executable paths and the local database
/// root are explicit dependencies so boundary tests do not mutate process-global
/// environment.
...
impl SystemPacman {
    pub fn new() -> Self {
        Self::with_paths(
            ...
            PathBuf::from("/usr/bin/pacman-conf"),
            None,
        )
    }

    /// Use an explicitly supplied pacman DB root instead of invoking
    /// `pacman-conf DBPath`. The path is the root containing the `local/`
    /// database directory, matching pacman's `DBPath` setting.
    ...
    /// Construct the adapter with all external boundary paths injected.
    pub fn with_paths(
        pacman_path: impl Into<PathBuf>,
        pacman_conf_path: impl Into<PathBuf>,
        db_path: Option<PathBuf>,
    ) -> Self {
        Self {
            pacman_path: pacman_path.into(),
            pacman_conf_path: pacman_conf_path.into(),
            db_path,
        }
    }
}
...
impl Pacman for SystemPacman {
    fn query(&self, name: &str) -> Option<String> {
        let out = Command::new(&self.pacman_path)
            .args(["-Q", "--", name])
            .output()
            .ok()?;
    ...
    fn local_record(&self, name: &str) -> Option<LocalRecord> {
        // Pacman's real DB has one currently installed <name>-<version>
        // directory per package name, so the first matching record is complete.
        // `FixturePacman` retains old records and deliberately chooses newest.
        let db_path = if let Some(db_path) = &self.db_path {
            db_path.clone()
        } else {
            let out = Command::new(&self.pacman_conf_path)
                .arg("DBPath")
                .output()
                .ok()?;
            if !out.status.success() {
...
mod tests {
    ...
    fn system_pacman_local_record_reads_real_layout() {
        ...
        let mismatch = local.join("pkg-0-1");
        let match_dir = local.join("pkg-1-2");
        ...
        let pacman = SystemPacman::with_db_path(Some(db));
        let rec = pacman
            .local_record("pkg")
        ...
        assert!(pacman.local_record("missing").is_none());
    }

    // --- SystemPacman subprocess seams: query / sync_info / dep_satisfied ---
    // Each adapter receives its fake executable directly, so these tests do not
    // serialize or mutate process-global environment.

    fn test_pacman(path: &Path) -> SystemPacman {
        SystemPacman::with_paths(
            path.to_path_buf(),
            PathBuf::from("/usr/bin/pacman-conf"),
            None,
        )
    ...
    fn system_pacman_query_invokes_pacman_and_maps_output() {
        ...
        let pacman = test_pacman(&fake);
        assert_eq!(
            pacman.query("ventoy-bin").as_deref(),
        ...
        let failing_pacman = test_pacman(&fail);
        assert!(failing_pacman.query("absent").is_none());
    }
    ...
    fn system_pacman_sync_info_invokes_pacman_and_maps_exit() {
        ...
        let pacman = test_pacman(&ok);
        assert!(pacman.sync_info("core/glibc"));
        ...
        write_exec(&fail, "#!/bin/sh\nexit 1\n");
        let failing_pacman = test_pacman(&fail);
        assert!(!failing_pacman.sync_info("aur/never"));
    }
    ...
    fn system_pacman_dep_satisfied_invokes_pacman_and_maps_exit() {
        ...
        let pacman = test_pacman(&satisfied);
        assert!(pacman.dep_satisfied("glibc>=2.38"));
        assert_logged_args(&log, &["-T", "--", "glibc>=2.38"]);

        let unsatisfied = temp.path().join("pacman-fail");
        write_exec(&unsatisfied, "#!/bin/sh\nexit 1\n");
        let failing_pacman = test_pacman(&unsatisfied);
        assert!(!failing_pacman.dep_satisfied("missing-lib"));
    }
